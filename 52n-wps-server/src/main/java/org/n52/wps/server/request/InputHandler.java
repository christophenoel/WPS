...
public class InputHandler {
    ...
    private InputHandler(Builder builder) throws ExceptionReport {
        ...
        this.processDesc = (ProcessDescriptionType) RepositoryManagerSingletonWrapper.getInstance().getProcessDescription(
                algorithmIdentifier).getProcessDescriptionType(
                        WPSConfig.VERSION_100);
        this.processOffering = (ProcessOffering) RepositoryManagerSingletonWrapper.getInstance().getProcessDescription(
                algorithmIdentifier).getProcessDescriptionType(
        ...
        Map<String, InterceptorInstance> inputInterceptors = resolveInputInterceptors(
                algorithmIdentifier);
        ...
        if (inputsV100 != null) {
            ...
            for (InputType input : inputsV100) {
                ...
                if (inputInterceptors.containsKey(inputId)) {
                    InterceptorInstance interceptor = inputInterceptors.get(
                            inputId);
                ...
                if (input.getData() != null) {
                    ...
                    throw new ExceptionReport(
                            "Error while accessing the inputValue: " + inputId,
                            ExceptionReport.INVALID_PARAMETER_VALUE);
                }
            }
            ...
            for (DataInputType input : inputsV200) {
                ...
                if (input.getData() != null) {
                    ...
                    net.opengis.wps.x20.InputDescriptionType inputDescription = XMLBeansHelper.findInputByID(
                            inputId, processOffering.getProcess());
    ...
    Map<String, InterceptorInstance> resolveInputInterceptors(
        ...
        try {
            clazz = Class.forName(algorithmClassName, false,
        ...
        DataInputInterceptorImplementations annotation = clazz.getAnnotation(
                DataInputInterceptors.DataInputInterceptorImplementations.class);
        if (annotation != null) {
            ...
            if (DataInputInterceptors.class.isAssignableFrom(interceptorClazz)) {
                ...
                try {
                    ...
                    LOGGER.warn(
                            "Could not instantiate class " + interceptorClazz, e);
                    return result;
    ...
    ComplexDataDescriptionType getNonDefaultFormat(
            InputDescriptionType inputRefDesc, String dataMimeType,
        ...
        try {
            ...
            if (complexValueNode.getChildNodes().getLength() > 1) {
                ...
                if (complexValue == null) {
                    return XMLUtil.nodeToString(
                            complexValueNode.getChildNodes().item(1));
                }
            } else {
            ...
            throw new TransformerFactoryConfigurationError(
                    "Could not parse inline data. Reason " + e1);
    ...
    protected void handleComplexData(InputType input, String inputId) throws ExceptionReport {
        ...
        inputReferenceDesc = XMLBeansHelper.findInputByID(inputId,
                processDesc.getDataInputs());
        if (inputReferenceDesc == null) {
            LOGGER.debug(
                    "Input cannot be found in description for " + processDesc.getIdentifier().getStringValue() + "," + inputId);
        }
        ...
        if (data.isSetMimeType() && dataMimeType != null) {
            format = findComplexDataDescriptionType(inputReferenceDesc,
                ...
                if (StringUtils.isBlank(dataMimeType)) {
                    ...
                    if (data.isSetEncoding() && !data.isSetSchema()) {
                        ...
                        /*
                             ...
                             * */
                        if (!StringUtils.isBlank(defaultSchema) && defaultSchema.equalsIgnoreCase(
                                data.getSchema())) {
                            ...
                            for (ComplexDataDescriptionType tempFormat : formats) {
                                 ...
                                 */
                                if (tempFormat.isSetSchema() && tempFormat.getSchema().equalsIgnoreCase(
                                        data.getSchema())) {
                                    foundSchema = tempFormat.getSchema();
                                    schemaFormat = tempFormat;
                                    found++;
                        ...
                        if (defaultEncoding.equalsIgnoreCase(data.getEncoding())) {
                            foundEncodingList.add(
                                    inputReferenceDesc.getComplexData().getDefault().getFormat());
                        } else {
                            ComplexDataDescriptionType[] formats = inputReferenceDesc.getComplexData().getSupported().getFormatArray();
                            for (ComplexDataDescriptionType tempFormat : formats) {
                                if (tempFormat.getEncoding().equalsIgnoreCase(
                            ...
                            if (defaultSchema != null && defaultSchema.equalsIgnoreCase(
                                ...
                                for (ComplexDataDescriptionType tempFormat : formats) {
                                     ...
                                     */
                                    if (tempFormat.getSchema() != null && tempFormat.getSchema().equalsIgnoreCase(
                                            data.getSchema())) {
                                        foundSchemaList.add(tempFormat);
                                    }
                                }
        ...
        try {
            ...
            Class<?> algorithmInput = RepositoryManagerSingletonWrapper.getInstance().getInputDataTypeForAlgorithm(
                    this.algorithmIdentifier, inputId);
        ...
        if (parser == null) {
            throw new ExceptionReport(
                    "Error. No applicable parser found for " + formatSchema + "," + dataMimeType + "," + formatEncoding,
    ...
    protected ComplexDataDescriptionType findComplexDataDescriptionType(
            InputDescriptionType inputReferenceDesc, String dataMimeType,
            String dataSchema, String dataEncoding, String potentialFormatSchema,
    ...
    private void handleComplexValueReference(DataInputType input)
        ...
        ReferenceStrategyRegister register = ReferenceStrategyRegister
                .getInstance();
        ReferenceInputStream stream = register.resolveReference(
                new InputReference(input));
    ...
    private Format getNonDefaultFormat(
            net.opengis.wps.x20.InputDescriptionType inputRefDesc,
            String dataMimeType,
            String dataSchema, String dataEncoding) {
        //TODO still needed?
//        if (inputRefDesc.getComplexData() == null) {
...
//        }
    ...
    protected IData parseComplexValue(String formatEncoding, String complexValue,
            String dataMimeType, String formatSchema, IParser parser) throws ExceptionReport {
        IData idata;
        ...
        if (StringUtils.isBlank(formatEncoding) || formatEncoding.equalsIgnoreCase(
                IOHandler.DEFAULT_ENCODING)) {
            try {
                if (!complexValueCopy.contains(
                        "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"")) {
                    complexValueCopy = complexValueCopy.replace(
                            "xsi:schemaLocation",
                            "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation");
                }
                idata = parser.parse(new ByteArrayInputStream(
                        complexValueCopy.getBytes()), dataMimeType, formatSchema);
            } catch (RuntimeException e) {
            ...
            idata = getBase64EncodedData(complexValue, parser, dataMimeType,
                    formatSchema);
        } else {
            throw new ExceptionReport(
                    "Unable to generate encoding " + formatEncoding,
                    ExceptionReport.NO_APPLICABLE_CODE);
        }
        return idata;
    }
    ...
    protected IData getBase64EncodedData(String complexValue, IParser parser,
            String dataMimeType, String formatSchema) throws ExceptionReport {
        File f = null;
        String complexValueCopy = complexValue.toString();
        ...
        try {
            ...
            return parser.parseBase64(new FileInputStream(f), dataMimeType,
                    formatSchema);
    ...
    private void handleLiteralData(InputType input) throws ExceptionReport {
        ...
        InputDescriptionType inputDesc = XMLBeansHelper.findInputByID(inputID,
                processDesc.getDataInputs());
        ...
        if (xmlDataType == null) {
            ...
            xmlDataType = xmlDataType.replace(
                    "http://www.w3.org/TR/xmlschema-2#", "xs:");
        }
        xmlDataType = xmlDataType.toLowerCase();

        IData parameterObj = null;
        try {
            parameterObj = BasicXMLTypeFactory.getBasicJavaObject(xmlDataType,
                    parameter);
        } catch (RuntimeException e) {
            throw new ExceptionReport(
                    "The passed parameterValue: " + parameter + " for input " + inputID + " is not of type: " + xmlDataType,
                    ExceptionReport.INVALID_PARAMETER_VALUE);
        }
        ...
        if (inputDesc.getLiteralData().isSetAllowedValues()) {
            if ((!inputDesc.getLiteralData().isSetAnyValue())) {
                ...
                for (ValueType allowedValue : allowedValues) {
                    if (input.getData().getLiteralData().getStringValue().equals(
                            allowedValue.getStringValue())) {
                ...
                if (parameterObj instanceof LiteralIntBinding || parameterObj instanceof LiteralDoubleBinding || parameterObj instanceof LiteralShortBinding || parameterObj instanceof LiteralFloatBinding || parameterObj instanceof LiteralLongBinding || parameterObj instanceof LiteralByteBinding) {
                    ...
                    for (RangeType allowedRange : allowedRanges) {
                        foundAllowedValue = checkRange(parameterObj,
                                allowedRange);
                    }
                }
                ...
                if (!foundAllowedValue && (allowedValues.length != 0 || allowedRanges.length != 0)) {
                    throw new ExceptionReport(
                            "Input with ID " + inputID + " does not contain an allowed value. See ProcessDescription.",
                            ExceptionReport.INVALID_PARAMETER_VALUE);
                }
        ...
        if (parameterObj == null) {
            throw new ExceptionReport(
                    "XML datatype as LiteralParameter is not supported by the server: dataType " + xmlDataType,
                    ExceptionReport.INVALID_PARAMETER_VALUE);
        }

        if (uom != null && !uom.equals("")) {
            if (parameterObj instanceof AbstractLiteralDataBinding) {
                ((AbstractLiteralDataBinding) parameterObj).setUnitOfMeasurement(
                        uom);
            }
        }
    ...
    private void handleLiteralData(DataInputType input) throws ExceptionReport {
        ...
        for (int i = 0; i < dataNode.getChildNodes().getLength(); i++) {
            ...
            if (childNode != null && localName != null && localName.equals(
                    LOCAL_NAME_LITERAL_VALUE)) {
                try {
                    literalValueDocument = LiteralValueDocument.Factory.parse(
                            childNode);
                    // set the parameter value here
                    parameter = literalValueDocument.getLiteralValue().getStringValue();
                    break;
                } catch (XmlException e) {
                    LOGGER.error("Exception while trying to parse LiteralValue.",
                            e);
                }
            }
        }
        // if parameter is empty try with text/plain mimeType (added this support for Testbed13 requirement)
        if (StringUtils.isEmpty(parameter)) {
            try {
                parameter = input.getData().getDomNode().getFirstChild().getNodeValue();
            } catch (Exception e) {
        ...
        net.opengis.wps.x20.InputDescriptionType inputDesc = XMLBeansHelper.findInputByID(
                inputID, processOffering.getProcess());

        LiteralDataDomain literalDataDomain = ((LiteralDataType) inputDesc.getDataDescription()).getLiteralDataDomainArray(
                0);
            ...
            if ((!literalDataDomain.isSetAnyValue())) {
                ...
                if (parameterObj instanceof LiteralIntBinding || parameterObj instanceof LiteralDoubleBinding || parameterObj instanceof LiteralShortBinding || parameterObj instanceof LiteralFloatBinding || parameterObj instanceof LiteralLongBinding || parameterObj instanceof LiteralByteBinding) {
                    ...
                    for (net.opengis.ows.x20.RangeType allowedRange : allowedRanges) {
                        foundAllowedValue = checkRangeV200(parameterObj,
                                allowedRange);
                    }
                }
    ...
    private boolean checkRange(IData parameterObj, RangeType allowedRange) {
        ...
        if (l == null || l.isEmpty() || l.get(0).equals("closed")) {
            ...
            if ((parameterObj instanceof LiteralIntBinding)) {
                int min = new Integer(
                        allowedRange.getMinimumValue().getStringValue());
                int max = new Integer(
                        allowedRange.getMaximumValue().getStringValue());
    ...
    private boolean checkRangeV200(IData parameterObj,
            net.opengis.ows.x20.RangeType allowedRange) {
    ...
    private void handleComplexValueReference(InputType input) throws ExceptionReport {
        ...
        if (inputPD == null) { // check if there is a corresponding input identifier in the process description
            LOGGER.debug(
                    "Input cannot be found in description for " + this.processDesc.getIdentifier().getStringValue() + "," + inputID);
            throw new RuntimeException(
                    "Input cannot be found in description for " + this.processDesc.getIdentifier().getStringValue() + "," + inputID);
        }
        ...
        if (referenceData.isSetMimeType() && referenceData.getMimeType() != null) {
            ...
            if (defaultMimeType.equalsIgnoreCase(mimeType)) {
                ...
                if (referenceData.getSchema() != null && referenceData.getEncoding() != null) {
                    if (referenceData.getSchema().equalsIgnoreCase(
                            potentialFormat.getSchema()) && referenceData.getEncoding().equalsIgnoreCase(
                            potentialFormat.getEncoding())) {
                        canUseDefault = true;
                        format = potentialFormat;
                    }
            ...
            if (format == null) {
                throw new ExceptionReport(
                        "Possibly multiple or none matching generators found for the input data with id = \"" + inputPD.getIdentifier().getStringValue() + "\". Is the MimeType (\"" + referenceData.getMimeType() + "\") correctly set?",
                        ExceptionReport.INVALID_PARAMETER_VALUE);
            ...
            if (mimeType != null) {
                ...
                if (!canUseDefault) {
                    ComplexDataDescriptionType[] formats = inputPD.getComplexData().getSupported().getFormatArray();
                    for (ComplexDataDescriptionType potentialFormat : formats) {
                        if (!StringUtils.isBlank(potentialFormat.getMimeType()) && potentialFormat.getMimeType().equalsIgnoreCase(
                                mimeType)) {
                            if (referenceData.getSchema() != null && referenceData.getEncoding() == null) {
                                if (referenceData.getSchema().equalsIgnoreCase(
                                        potentialFormat.getSchema())) {
                ...
                if (format == null) {
                    ...
                    // TODO Review error message
                    throw new ExceptionReport(
                            "Could not determine input format because none of the supported formats match the given schema (\"" + referenceData.getSchema() + "\") and encoding (\"" + referenceData.getEncoding() + "\"). (A mimetype was not specified)",
                            ExceptionReport.INVALID_PARAMETER_VALUE);
            ...
            if (mimeType == null && !referenceData.isSetEncoding() && !referenceData.isSetSchema()) {
                ...
                if (mimeType == null) {
                    if (referenceData.isSetEncoding() && !referenceData.isSetSchema()) {
                        ...
                        int found = 0;
                        String foundEncoding = null;
                        if (defaultEncoding.equalsIgnoreCase(
                                referenceData.getEncoding())) {
                    ...
                    if (referenceData.isSetSchema() && !referenceData.isSetEncoding()) {
                        ...
                        int found = 0;
                        String foundSchema = null;
                        if (defaultSchema.equalsIgnoreCase(
                                referenceData.getSchema())) {
                    ...
                    if (referenceData.isSetEncoding() && referenceData.isSetSchema()) {
                        ...
                        if (defaultEncoding.equalsIgnoreCase(
                                referenceData.getEncoding())) {
                            foundEncodingList.add(
                                    inputPD.getComplexData().getDefault().getFormat());
        ...
        IParser parser = null;
        try {
            Class<?> algorithmInputClass = RepositoryManagerSingletonWrapper.getInstance().getInputDataTypeForAlgorithm(
                    this.algorithmIdentifier, inputID);
            if (algorithmInputClass == null) {
                throw new RuntimeException(
                        "Could not determine internal input class for input" + inputID);
            }
            ...
            if (parser == null) {
                throw new ExceptionReport(
                        "Error. No applicable parser found for schema=\"" + schema + "\", mimeType=\"" + mimeType + "\", encoding=\"" + encoding + "\"",
                        ExceptionReport.NO_APPLICABLE_CODE);
            }
        } catch (RuntimeException e) {
            throw new ExceptionReport("Error obtaining input data",
...
//            }
    ...
    private IData parseBoundingBox(BoundingBoxType bbt)
        ...
        if (dim != null && (dim.compareTo(INT_MAX) > 0
            ...
            throw new ExceptionReport(
                    String.format(
                            "Unsupported BoundingBox dimension %s. Has to be betweeen %s and %s!",
                            dim, INT_MIN, INT_MAX),
                    ExceptionReport.INVALID_PARAMETER_VALUE);
        }
        ...
        if (upper.length != lower.length) {
            throw new ExceptionReport(
                    String.format(
                            "Mismatching BoundingBox dimensions: %s vs %s!",
    ...
    private void handleBBoxValue(DataInputType input)
        ...
        try {
            if (dataNode.getChildNodes().getLength() > 1) {
                boundingBoxDocument = BoundingBoxDocument.Factory.parse(
                        dataNode.getChildNodes().item(1));
            } else {
                boundingBoxDocument = BoundingBoxDocument.Factory.parse(
                        input.getData().getDomNode());
            }
        } catch (XmlException e) {
            LOGGER.error(
                    "XmlException occurred while trying to parse bounding box: " + (boundingBoxDocument == null ? null : boundingBoxDocument.toString()),
                    e);
        }
