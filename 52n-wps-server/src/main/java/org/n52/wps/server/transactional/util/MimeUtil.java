...
public class MimeUtil {
    ...
    private static HashMap<String, String> extensionMapping;

    static {
        extensionMapping = new HashMap<>(200);
        for (Map.Entry<String, String> entry : mimeTypeMapping.entrySet()) {
            extensionMapping.put(entry.getValue(), entry.getKey());
        }
    }
    ...
    public static String lookupMimeType(String ext) {
       ...
       /**
     * Simply returns EXTENSION or <code>null</code> if no type is found.
     */
    public static String lookupExtension(String mime) {
        return extensionMapping.get(mime.toLowerCase());
    }
    
       /**
     * Returns the corresponding EXTENSION to the given mime. If no Extension
     * type was found it returns 'application/octet-stream' type.
     */
    public static String getExtension(String mime) {
        String extension = lookupExtension(mime);
        if (extension == null) {
            extension = "";
        }
        return extension;
